...
%            [x , px (1.0 == the middle of the top left pixel, increasing right);
%             y , px (1.0 == the middle of the top left pixel, increasing down);
...
if isempty(nhoodctrs)
    ...
    if ~strcmpi(processopt, 'none')
    % calcthreshpts should dilate the processed image, even if a dilated 
    % image is input.
        if ~isempty(dimg)
            disp('for_rp.m: Ignoring input dilated image.')
        end
        dimg = [];
    end
...
Lx = lsumx(end);
Ly = lsumy(end);
meand2 = zeros(1, length(x));  % this will only be filled in for 'radia' localization
